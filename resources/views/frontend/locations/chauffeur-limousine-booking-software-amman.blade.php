...
@section('title', !empty($headerdata->meta_title) ? $headerdata->meta_title : 'Chauffeur and limousine booking software in Amman, Jordan | Edion Web Technologies')
@section('description', !empty($headerdata->meta_description) ? $headerdata->meta_description : 'Chauffeur and limousine booking software for businesses in Amman, Jordan. If you dispatch drivers as well as rent vehicles, that is genuinely a different product: driver rostering')
@section('keywords', !empty($headerdata->meta_keywords) ? $headerdata->meta_keywords : 'chauffeur and limousine booking software amman, chauffeur and limousine booking software jordan, chauffeur limousine booking software amman, chauffeur and limousine booking software company amman, best chauffeur and limousine booking software amman')
...
<section class="phero shell">
 <nav aria-label="Breadcrumb"><ol class="crumbs"><li><a href="{{ url('/') }}">Home</a></li><li><a href="{{ route('industry.detail', ['slug'=>'locations']) }}">Locations</a></li><li aria-current="page"><b class="kw2">Chauffeur and limousine</b> booking software in Amman</li></ol></nav>
 <p class="eyebrow">Chauffeur &middot; Jordan</p>
 <h1 style="margin-top:var(--sp-4)">Chauffeur and limousine booking software in Amman</h1>
 <p class="lead">If you dispatch drivers as well as rent vehicles, that is genuinely a different product: driver rostering, live dispatch, route-based trip pricing and driver settlements. It shares your <b class="kw2">fleet register</b> and nothing else. We work with businesses in <span class="kw kw--loc">Amman</span> and across <span class="kw kw--loc">Jordan</span>.</p>
...
<section class="band band--ink">
 ...
 <div><p class="lbl">Other services in Amman</p><ul><li><a href="{{ route('industry.detail', ['slug'=>'car-rental-software-development-amman']) }}">Car rental software development</a></li><li><a href="{{ route('industry.detail', ['slug'=>'fleet-management-software-amman']) }}">Fleet management software</a></li><li><a href="{{ route('industry.detail', ['slug'=>'car-rental-booking-engine-amman']) }}">Car rental booking engine</a></li><li><a href="{{ route('industry.detail', ['slug'=>'vehicle-rental-app-development-amman']) }}">Vehicle rental app development</a></li><li><a href="{{ route('industry.detail', ['slug'=>'website-development-amman']) }}">Website development</a></li><li><a href="{{ route('industry.detail', ['slug'=>'custom-software-development-amman']) }}">Custom software development</a></li></ul></div>
