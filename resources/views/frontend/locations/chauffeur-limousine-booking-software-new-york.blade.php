...
@section('title', !empty($headerdata->meta_title) ? $headerdata->meta_title : 'Chauffeur and limousine booking software in New York, United States | Edion Web Technologies')
@section('description', !empty($headerdata->meta_description) ? $headerdata->meta_description : 'Chauffeur and limousine booking software for businesses in New York, United States. If you dispatch drivers as well as rent vehicles, that is genuinely a different product: driver rostering')
@section('keywords', !empty($headerdata->meta_keywords) ? $headerdata->meta_keywords : 'chauffeur and limousine booking software new york, chauffeur and limousine booking software united states, chauffeur limousine booking software new york, chauffeur and limousine booking software company new york, best chauffeur and limousine booking software new york')
...
<section class="phero shell">
 <nav aria-label="Breadcrumb"><ol class="crumbs"><li><a href="{{ url('/') }}">Home</a></li><li><a href="{{ route('industry.detail', ['slug'=>'locations']) }}">Locations</a></li><li aria-current="page"><b class="kw2">Chauffeur and limousine</b> booking software in New York</li></ol></nav>
 <p class="eyebrow">Chauffeur &middot; United States</p>
 <h1 style="margin-top:var(--sp-4)">Chauffeur and limousine booking software in New York</h1>
 <p class="lead">If you dispatch drivers as well as rent vehicles, that is genuinely a different product: driver rostering, live dispatch, route-based trip pricing and driver settlements. It shares your <b class="kw2">fleet register</b> and nothing else. We work with businesses in <span class="kw kw--loc">New York</span> and across United States.</p>
...
<section class="band band--ink">
 ...
 <div><p class="lbl">North America</p><ul><li><a href="{{ route('industry.detail', ['slug'=>'chauffeur-limousine-booking-software-toronto']) }}">Toronto</a></li></ul></div>
 <div><p class="lbl">Other services in New York</p><ul><li><a href="{{ route('industry.detail', ['slug'=>'car-rental-software-development-new-york']) }}">Car rental software development</a></li><li><a href="{{ route('industry.detail', ['slug'=>'fleet-management-software-new-york']) }}">Fleet management software</a></li><li><a href="{{ route('industry.detail', ['slug'=>'car-rental-booking-engine-new-york']) }}">Car rental booking engine</a></li><li><a href="{{ route('industry.detail', ['slug'=>'vehicle-rental-app-development-new-york']) }}">Vehicle rental app development</a></li><li><a href="{{ route('industry.detail', ['slug'=>'website-development-new-york']) }}">Website development</a></li><li><a href="{{ route('industry.detail', ['slug'=>'custom-software-development-new-york']) }}">Custom software development</a></li></ul></div>
