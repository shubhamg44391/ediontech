...
@section('title', !empty($headerdata->meta_title) ? $headerdata->meta_title : 'Chauffeur and limousine booking software in Toronto, Canada | Edion Web Technologies')
@section('description', !empty($headerdata->meta_description) ? $headerdata->meta_description : 'Chauffeur and limousine booking software for businesses in Toronto, Canada. If you dispatch drivers as well as rent vehicles, that is genuinely a different product: driver rostering')
@section('keywords', !empty($headerdata->meta_keywords) ? $headerdata->meta_keywords : 'chauffeur and limousine booking software toronto, chauffeur and limousine booking software canada, chauffeur limousine booking software toronto, chauffeur and limousine booking software company toronto, best chauffeur and limousine booking software toronto')
...
<section class="phero shell">
 <nav aria-label="Breadcrumb"><ol class="crumbs"><li><a href="{{ url('/') }}">Home</a></li><li><a href="{{ route('industry.detail', ['slug'=>'locations']) }}">Locations</a></li><li aria-current="page"><b class="kw2">Chauffeur and limousine</b> booking software in Toronto</li></ol></nav>
 <p class="eyebrow">Chauffeur &middot; Canada</p>
 <h1 style="margin-top:var(--sp-4)">Chauffeur and limousine booking software in Toronto</h1>
 <p class="lead">If you dispatch drivers as well as rent vehicles, that is genuinely a different product: driver rostering, live dispatch, route-based trip pricing and driver settlements. It shares your <b class="kw2">fleet register</b> and nothing else. We work with businesses in <span class="kw kw--loc">Toronto</span> and across Canada.</p>
...
<section class="band band--ink">
 ...
 <div><p class="lbl">North America</p><ul><li><a href="{{ route('industry.detail', ['slug'=>'chauffeur-limousine-booking-software-new-york']) }}">New York</a></li></ul></div>
 <div><p class="lbl">Other services in Toronto</p><ul><li><a href="{{ route('industry.detail', ['slug'=>'car-rental-software-development-toronto']) }}">Car rental software development</a></li><li><a href="{{ route('industry.detail', ['slug'=>'fleet-management-software-toronto']) }}">Fleet management software</a></li><li><a href="{{ route('industry.detail', ['slug'=>'car-rental-booking-engine-toronto']) }}">Car rental booking engine</a></li><li><a href="{{ route('industry.detail', ['slug'=>'vehicle-rental-app-development-toronto']) }}">Vehicle rental app development</a></li><li><a href="{{ route('industry.detail', ['slug'=>'website-development-toronto']) }}">Website development</a></li><li><a href="{{ route('industry.detail', ['slug'=>'custom-software-development-toronto']) }}">Custom software development</a></li></ul></div>
