...
@section('title', !empty($headerdata->meta_title) ? $headerdata->meta_title : 'Technical SEO audit for healthcare | Edion Web Technologies')
@section('description', !empty($headerdata->meta_description) ? $headerdata->meta_description : 'Technical SEO audit for healthcare businesses. Health content sits under stricter quality assessment than most sectors, so authorship, credentials and citati')
@section('keywords', !empty($headerdata->meta_keywords) ? $headerdata->meta_keywords : 'technical seo audit for healthcare, healthcare technical seo audit, technical seo audit healthcare, healthcare software company, technical seo audit company')
...
<section class="phero shell">
  <nav aria-label="Breadcrumb"><ol class="crumbs"><li><a href="{{ url('/') }}">Home</a></li><li><a href="{{ route('industry.detail', ['slug'=>'industries']) }}">Industries</a></li><li><a href="{{ route('industry.detail', ['slug'=>'software-for-healthcare']) }}">Healthcare</a></li><li aria-current="page"><b class="kw">Technical SEO audit</b></li></ol></nav>
  <span class="ind-badge" style="display:inline-flex">
    <img src="{{ asset('assets/frontend/img/industry/healthcare.svg') }}" alt="" width="34" height="34" loading="eager">
    <span>Healthcare</span></span>
  <h1 style="margin-top:var(--sp-4)">Technical SEO audit for healthcare</h1>
  <p class="lead">A prioritised fix list with effort estimates, not a 90-page PDF. This page covers what changes when the client is a healthcare business,
...
<section class="band band--tight shell">
  <div class="lp-facts" data-reveal>
    <div class="lp-fact"><b>Healthcare</b><span>Sector experience</span></div>
...
<section class="band shell">
  <div class="article">
    <div class="article__body"><h2>Why healthcare needs a different approach</h2><p>Healthcare providers handle sensitive personal data under stricter consent and retention expectations than most sectors, and their patients are often not confident with technology.</p><h2>Technical SEO audit in this sector, specifically</h2><p>Health content sits under stricter quality assessment than most sectors, so authorship, credentials and citation matter unusually much. We build author pages, reference clinical sources and avoid the claim language that gets pages suppressed.</p><h2>Where we start</h2><p>For healthcare projects the first release centres on access control, audit logging, retention policy and consent capture. Those come first because they change how the business runs rather than how it looks. Visual refinement and secondary modules follow once the core is in real use, which is also when you learn which of them you actually need.</p><h2>How the engagement runs</h2><p>Weekly written updates, a staging URL open every day, and one accountable point of contact rather than a handoff chain. You review working software at each increment, so there is no reveal at the end where unpleasant surprises live.</p></div>
    <aside class="aside">
      ...
      <div class="card" style="padding:var(--sp-5)">
        <p class="card__idx">Healthcare</p>
        <h3 style="font-size:var(--fs-h4)">All services for this sector</h3>
        <p>See every service we deliver to healthcare businesses.</p>
        <a class="btn btn--line" href="{{ route('industry.detail', ['slug'=>'software-for-healthcare']) }}" style="justify-content:center">View sector<svg class="btn__arrow" viewBox="0 0 16 16" fill="none" aria-hidden="true"><path d="M2 8h12M9 3l5 5-5 5" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg></a>
      </div>
    </aside>
  </div>
</section>
...
<section class="band band--ink">
  <div class="shell">
    <div class="sec-head" data-reveal>
      <div><p class="eyebrow">Questions</p><h2>Technical SEO audit for healthcare: common questions</h2></div>
    </div>
